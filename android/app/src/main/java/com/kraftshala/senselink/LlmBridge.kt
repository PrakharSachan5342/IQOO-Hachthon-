...
/**
 * Native on-device LLM bridge exposed to the WebView as `window.AndroidLLM`.
 *
 * Runs Google **Gemma-2B (int4)** locally via the **LiteRT / MediaPipe LLM Inference** runtime.
 * The model is NOT shipped in the APK (it's ~1.3 GB); it is provisioned on first run by either:
 *   - side-loading:  adb push gemma.task  /sdcard/Android/data/com.kraftshala.senselink/files/
 *   - download:      AndroidLLM.ensureModel(url)  (streams to filesDir/llm/gemma.task)
 *
 * If the model is absent or fails to load, `isReady()` returns false and the web app falls back
 * to its deterministic rule-based composer — so the demo never blocks on the model.
 */
class LlmBridge(
    ...
    /** Prefer an adb-pushable external file, else the private downloaded copy. */
    ...
    /** Rewrite gesture tokens into a fluent sentence. Result is delivered to window.__onLLM(id, text). */
    @JavascriptInterface
    ...
    /** Best-effort first-run download of the model to private storage. */
    @JavascriptInterface
    fun ensureModel(url: String) {
        if (modelFile().exists()) { initIfPresent(); return }
        if (url.isBlank()) { progress(-1, "NO MODEL URL SET"); return }
        worker.execute {
            val dir = File(activity.filesDir, "llm").apply { mkdirs() }
                    ...
                    tmp.outputStream().use { out ->
                        ...
                        while (input.read(buf).also { read = it } >= 0) {
                            ...
                            if (total > 0) {
                                ...
                                if (pct != lastPct) { lastPct = pct; progress(pct, "DOWNLOADING GEMMA…") }
                            }
                        }
                    }
                }
                if (tmp.renameTo(dest)) { progress(100, "MODEL READY"); initIfPresent() }
                else { tmp.delete(); progress(-1, "SAVE FAILED") }
            } catch (e: Throwable) {
                tmp.delete()
