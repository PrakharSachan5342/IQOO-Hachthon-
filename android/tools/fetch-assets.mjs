/**
 * fetch-assets.mjs — produces the fully-offline asset bundle for the SENSE-LINK APK.
 *
 * Downloads (into app/src/main/assets/www/):
 *   - MediaPipe tasks-vision ESM bundle + WASM runtime (SIMD + no-SIMD)
 *   - the hand_landmarker .task model
 *   - the Google Fonts (Anton / Archivo / Space Mono) woff2 files + a local fonts.css
 * Then copies the repo-root index.html into assets and rewrites every CDN URL to a
 * local relative path, so the app runs in true airplane mode.
 *
 * Run from the android/ dir:  node tools/fetch-assets.mjs
 */
